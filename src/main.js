// No critical bugs, but the dep object is created unconditionally at module load.
// If any env var is missing, services fail with cryptic errors.
// FIX: added a startup env check so failures are obvious immediately.
import CICDExecution from "../modules/cicdexecution.js";
import FailureClassifier from "../modules/failureclassifier.js";
import RecoverManager from "../modules/recoveryManager.js";
import NotificationService from "../modules/notificationService.js";
import Logger from "../modules/logger.js";
import MonitoringService from "../modules/monitoringService.js";
//import dotenv from "dotenv";
import executePipeline from "./bll/pipelineManager.js";

//dotenv.config();

// FIX: fail fast if required env vars are missing
// const required = ["EMAIL_USER", "EMAIL_PASS", "EMAIL_TO", "DB_USER", "DB_PASS"];
// for (const key of required) {
//   if (!process.env[key]) {
//     console.error(`Missing required env var: ${key}`);
//     process.exit(1);
//   }
// }

const dep = {
  pipeline: new CICDExecution("src/app.py"),
  classifier: new FailureClassifier(),
  recovery: new RecoverManager(),
  notification: new NotificationService(),
  logger: new Logger(),
  monitor: new MonitoringService(),
};

export default async function startPipeline() {
  try {
    const result = await executePipeline(dep);
    return result;
  } catch (error) {
    console.error("Pipeline Crash:", error.message); // FIX: console.error, not .log
    return { status: "ERROR", error: error.message };
  }
}